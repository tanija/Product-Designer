...
function wcpd_product_desingner(){
    ...
    $html.='<div id="canvas">';
    $divproducts ='<div class="canvas" id="products"><h5>Products:  </h5>'.product_designer_get_product_list().'</div>';
    $div_playground = '<div class="canvas" id="product-page"><div class="style-wrapper"><div class="styles-list"></div></div><div id="canvas-wrapper"><div class="product-name"></div><canvas id="playground" width=500 height=500 style="position: relativ; z-index: 5"></canvas><canvas id="knobs-playground" width=500 height=500 style="position: absolute; z-index: 10"></canvas></div><div id="hidden1"></div><div id="product-cart-info"><div id="info"><div class="product-price"></div></div><div id="cart-button"></div></div><div id="hidden2">'.__("Enjoy with our app").'</div></div>';
    $div_accessoires = '<div class="canvas" id="accessoires">'.__("<h5>Knobs... </h5>").'<div>'.__("<h5>...by size: </h5>").product_designer_get_accessoires_cat1().'</div><div>'.__("<h5>...by style: </h5>").product_designer_get_accessoires_cat2().'</div>'
        .product_designer_get_accessoires_list_by_sel_cat().'</div>';
    
    $html.= $divproducts.$div_playground.$div_accessoires;
    $html.='</div>';
    return $html;
}
...
function product_designer_get_product_list()
	{		
		...
		if($prodict_query->have_posts()): while($prodict_query->have_posts()): $prodict_query->the_post();                    $sticker_url = wp_get_attachment_url( get_post_thumbnail_id(get_the_ID()) );                           
			if(!empty($sticker_url))
				{
					$html.= '<img id="'.get_the_ID().'" src="'.$sticker_url.'"/>';               
				}				
    ...
    //$html .= '<h4>Styles: </h4>';
        ...
        foreach ( $attachments as $attachment ) { 
           $url = wp_get_attachment_url( $attachment->ID, 300, 300 );
            if(!(preg_match('/featured.png/',$url))){
...
function product_designer_get_accessoires_list()
	{		
...
function product_designer_get_accessoires_list_by_sel_cat()
	{
	$args_prod = array(
...
function product_designer_get_accessoires_list_by_cat(){
    ...
    if (isset($_POST['term2'])) {
       $term2 = $_POST['term2'];
		...
		$html = '';
		//$html.='<div class="accessoires-list">';
...
function product_designer_get_accessoires_cat1()
	{
		...
		$html = '';       
		$html .= '<select class="accessoires-cat1" name="categories">';   
...
function product_designer_get_accessoires_cat2()
	{
		...
		$html = '';       
		$html .= '<select class="accessoires-cat2" name="categories">';   
